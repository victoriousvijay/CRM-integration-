...
@section('title', __('Edit Sequence'))
@section('page-title', __('Edit Drip Sequence'))

@section('breadcrumbs')
<li class="breadcrumb-item"><a href="{{ route('sequences.index') }}">{{ __('Sequences') }}</a></li>
<li class="breadcrumb-item"><a href="{{ route('sequences.show', $sequence) }}">{{ $sequence->name }}</a></li>
...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Edit: {{ $sequence->name }}</h3>
    </div>
    <div class="card-body">
        <form action="{{ route('sequences.update', $sequence) }}" method="POST" id="sequence-form">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $sequence->name) }}" required>
            ...
            <div id="steps-container">
                @php $steps = old('steps', $sequence->steps->map(function($s) {
                    return ['delay_days' => $s->delay_days, 'action_type' => $s->action_type, 'message_template' => $s->message_template];
                })->toArray()); @endphp
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                ...
                                <input type="number" name="steps[{{ $i }}][delay_days]" class="form-control @error('steps.'.$i.'.delay_days') is-invalid @enderror" value="{{ $step['delay_days'] ?? 0 }}" min="0" required>
            ...
            <div class="card-footer text-end">
                <a href="{{ route('sequences.show', $sequence) }}" class="btn btn-outline-secondary me-2">{{ __('Cancel') }}</a>
                <button type="submit" class="btn btn-primary">{{ __('Update Sequence') }}</button>
            </div>
        </form>
    </div>
</div>
...
document.getElementById('steps-container').addEventListener('click', function(e) {
    ...
    // Get previous step summary if available
