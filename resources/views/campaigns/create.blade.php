...
@section('page-title', __('Create Campaign'))
...
<form action="{{ route('campaigns.store') }}" method="POST">
        ...
        <div class="card-body">
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}" required placeholder="{{ __('e.g., Spring 2026 Direct Mail') }}">
            ...
            <div class="row mb-3">
                <div class="col-md-4">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" name="budget" class="form-control @error('budget') is-invalid @enderror" value="{{ old('budget') }}" step="0.01" min="0" placeholder="0.00">
                    </div>
                ...
                <div class="col-md-4">
                    ...
                    <div class="input-group">
                        <span class="input-group-text">$</span>
                        <input type="number" name="actual_spend" class="form-control @error('actual_spend') is-invalid @enderror" value="{{ old('actual_spend', '0') }}" step="0.01" min="0" placeholder="0.00">
                    </div>
                    @error('actual_spend') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-4">
                    <label class="form-label">{{ __('Target Lead Count') }}</label>
                    <input type="number" name="target_count" class="form-control @error('target_count') is-invalid @enderror" value="{{ old('target_count', '0') }}" min="0">
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            ...
            <textarea name="notes" id="campaign-notes" class="form-control @error('notes') is-invalid @enderror" rows="4" placeholder="{{ __('Campaign description, goals, target audience, etc.') }}">{{ old('notes') }}</textarea>
            @error('notes') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>
    </div>

    <div class="d-flex justify-content-between">
        <a href="{{ route('campaigns.index') }}" class="btn btn-outline-secondary">{{ __('Cancel') }}</a>
