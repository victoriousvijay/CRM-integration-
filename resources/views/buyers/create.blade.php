...
@php $buyerLabel = $modeTerms['buyer_label'] ?? __('Cash Buyers'); @endphp
@section('title', ($businessMode ?? 'wholesale') === 'realestate' ? __('Add Client') : __('Add Cash Buyer'))
@section('page-title', ($businessMode ?? 'wholesale') === 'realestate' ? __('Add Client') : __('Add Cash Buyer'))

@section('breadcrumbs')
<li class="breadcrumb-item"><a href="{{ route('buyers.index') }}">{{ $buyerLabel }}</a></li>
<li class="breadcrumb-item active" aria-current="page">{{ ($businessMode ?? 'wholesale') === 'realestate' ? __('Add Client') : __('Add Buyer') }}</li>
@endsection

@section('content')
<div class="card">
    <div class="card-header">
        <h3 class="card-title">{{ ($businessMode ?? 'wholesale') === 'realestate' ? __('Client Information') : __('Buyer Information') }}</h3>
    </div>
    <div class="card-body">
        <form action="{{ route('buyers.store') }}" method="POST">
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" name="max_purchase_price" class="form-control @error('max_purchase_price') is-invalid @enderror" value="{{ old('max_purchase_price') }}" step="0.01" min="0">
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <label class="form-check">
                            <input type="checkbox" name="preferred_property_types[]" value="{{ $val }}" class="form-check-input" {{ in_array($val, old('preferred_property_types', [])) ? 'checked' : '' }}>
            ...
            <div class="mb-3">
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <label class="form-check">
                            <input type="checkbox" name="asset_classes[]" value="{{ $val }}" class="form-check-input" {{ in_array($val, old('asset_classes', [])) ? 'checked' : '' }}>
                            <span class="form-check-label">{{ $label }}</span>
                        </label>
                    </div>
                    @endforeach
            ...
            <div class="mb-3">
                ...
                <textarea name="notes" id="buyer-notes" class="form-control @error('notes') is-invalid @enderror" rows="4">{{ old('notes') }}</textarea>
                @error('notes') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>
            <div class="card-footer text-end">
                <a href="{{ route('buyers.index') }}" class="btn btn-outline-secondary me-2">{{ __('Cancel') }}</a>
                <button type="submit" class="btn btn-primary">{{ ($businessMode ?? 'wholesale') === 'realestate' ? __('Create Client') : __('Create Buyer') }}</button>
            </div>
        </form>
    </div>
</div>
