{{-- Comparable Market Analysis (CMA) Worksheet Card --}}
{{-- Include: @include('properties._cma_worksheet', ['property' => $property]) --}}
...
@php
    ...
    $avgAdjusted = $compCount > 0 ? $adjustedPrices->avg() : 0;
    $medianAdjusted = 0;
    if ($compCount > 0) {
        $sorted = $adjustedPrices->sort()->values();
        if ($compCount % 2 === 0) {
            $medianAdjusted = ($sorted[$compCount / 2 - 1] + $sorted[$compCount / 2]) / 2;
        } else {
            $medianAdjusted = $sorted[intdiv($compCount, 2)];
        }
    }

    // Price per sqft: avg adjusted_price / avg sqft of comps (only if sqft data available)
    $compSqfts = $comps->pluck('sqft')->filter();
    $avgSqft = $compSqfts->count() > 0 ? $compSqfts->avg() : 0;
    $pricePerSqft = ($avgSqft > 0 && $avgAdjusted > 0) ? $avgAdjusted / $avgSqft : 0;

    // Suggested list price range: median -3% to +3%
    $suggestedLow = $compCount > 0 ? $medianAdjusted * 0.97 : 0;
    $suggestedHigh = $compCount > 0 ? $medianAdjusted * 1.03 : 0;

    // Price positioning badge
    $listPrice = (float) ($property->list_price ?? $property->asking_price ?? 0);
    $pricePosition = null;
    if ($listPrice > 0 && $compCount > 0) {
        if ($listPrice >= $suggestedLow && $listPrice <= $suggestedHigh) {
            $pricePosition = 'competitive';
        } elseif ($listPrice > $suggestedHigh) {
        ...
        <div class="row row-cards mb-3">
            <div class="col-sm-6 col-lg-3">
                <div class="card card-sm">
                    <div class="card-body">
                        <div class="text-secondary small">{{ __('Avg Adjusted Price') }}</div>
                        <div class="fw-bold fs-3">{{ Fmt::currency($avgAdjusted, 0) }}</div>
                    </div>
                </div>
            </div>
            <div class="col-sm-6 col-lg-3">
                <div class="card card-sm">
                    <div class="card-body">
                        <div class="text-secondary small">{{ __('Median Adjusted Price') }}</div>
                        <div class="fw-bold fs-3">{{ Fmt::currency($medianAdjusted, 0) }}</div>
                    </div>
                </div>
            </div>
            <div class="col-sm-6 col-lg-3">
                <div class="card card-sm">
                    <div class="card-body">
                        <div class="text-secondary small">{{ __('Price Per Sqft') }}</div>
                        <div class="fw-bold fs-3">{{ $pricePerSqft > 0 ? Fmt::currency($pricePerSqft, 0) : '-' }}</div>
                    </div>
                </div>
            </div>
            <div class="col-sm-6 col-lg-3">
                <div class="card card-sm">
                    <div class="card-body">
                        <div class="text-secondary small">{{ __('Suggested List Price') }}</div>
                        <div class="fw-bold fs-4">{{ Fmt::currency($suggestedLow, 0) }} &ndash; {{ Fmt::currency($suggestedHigh, 0) }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                @if($listPrice > 0)
                    ...
                    <strong>{{ Fmt::currency($listPrice) }}</strong>
                    @if($pricePosition === 'competitive')
                        <span class="badge bg-green-lt ms-2">{{ __('Competitive Price') }}</span>
                    @elseif($pricePosition === 'premium')
                        <span class="badge bg-yellow-lt ms-2">{{ __('Premium Price') }}</span>
                    @elseif($pricePosition === 'below_market')
                        <span class="badge bg-blue-lt ms-2">{{ __('Below Market') }}</span>
                    @endif
                @endif
            </div>
            <div class="col-md-6">
                @if($pricePerSqft > 0)
                    <span class="text-secondary">{{ __('Avg Comp Sqft:') }}</span>
                    <strong>{{ number_format($avgSqft, 0) }}</strong>
                @endif
            </div>
        </div>

        {{-- CMA Chart --}}
        <div id="arv-chart" style="height:250px;" class="mb-3" role="img" aria-label="{{ __('Bar chart showing comparable sale adjusted prices with suggested list price range') }}"></div>
        @else
        <div class="text-center text-secondary py-3">
            <p>{{ __('No comparable sales added yet. Add comps below to calculate the CMA.') }}</p>
        </div>
        @endif
    </div>
</div>
    ...
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{ __('AI Market Analysis') }}</h5>
            ...
            <div class="modal-footer">
                <div id="arv-ai-apply-section" style="display:none;" class="me-auto">
                    <div class="input-group input-group-sm">
                        <span class="input-group-text">{{ __('List Price') }}</span>
                        <input type="number" class="form-control" id="arv-ai-apply-value" step="0.01" min="0" style="max-width:140px;">
                        <button type="button" class="btn btn-success" id="arv-ai-apply-btn">{{ __('Apply List Price') }}</button>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    (function() {
        ...
        document.getElementById('arv-ai-btn').addEventListener('click', function() {
            ...
            .then(function(res) {
                ...
                document.getElementById('arv-ai-apply-value').value = {{ $avgAdjusted > 0 ? round($avgAdjusted) : 0 }};
            })
            .catch(function() {
                document.getElementById('arv-ai-loading').style.display = 'none';
                document.getElementById('arv-ai-error').style.display = 'block';
        ...
        document.getElementById('arv-ai-apply-btn').addEventListener('click', function() {
            ...
            if (!arvValue || parseFloat(arvValue) <= 0) { alert('{{ __("Enter a valid list price.") }}'); return; }
            btn.disabled = true;
            btn.textContent = '{{ __("Applying...") }}';
            fetch('{{ url("/ai/apply-property-field") }}', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' },
                body: JSON.stringify({ property_id: propertyId, field: 'list_price', value: arvValue })
    ...
    var suggestedLow = {{ $suggestedLow }};
    var suggestedHigh = {{ $suggestedHigh }};
                ...
                yaxis: [
                    {
                        ...
                        label: {
                            ...
                            text: '{{ __("Suggested Range") }}: ' + currencySymbol + suggestedLow.toLocaleString(jsLocale, { maximumFractionDigits: 0 }) + ' - ' + currencySymbol + suggestedHigh.toLocaleString(jsLocale, { maximumFractionDigits: 0 }),
                            position: 'front'
                        }
                    }
                ]
